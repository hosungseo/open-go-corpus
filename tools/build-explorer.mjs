...
if (!WITH_NAMES) {
  ...
  for (const name of ["index", "compare", "history", "policy2", "notes", "report"]) {
    ...
    if (page.includes("/*__VSDATA__*/")) {
      if (!fs.existsSync(VS_PATH)) { console.error("ERROR notes/vision.json missing"); process.exit(1); }
      const vsd = fs.readFileSync(VS_PATH, "utf8").replace(/<\//g, "<\\/");
      page = page.replace("/*__VSDATA__*/null", () => vsd).replace("/*__VSDATA__*/", () => vsd);
    }
    ...
    if (page.includes("/*__LNDATA__*/")) {
      ...
      const ln = fs.readFileSync(LN_PATH, "utf8").replace(/<\//g, "<\\/");
      page = page.replace("/*__LNDATA__*/null", () => ln).replace("/*__LNDATA__*/", () => ln);
    }
    ...
    if (page.includes("/*__FLDATA__*/")) {
      ...
      const fl = fs.readFileSync(FL_PATH, "utf8").replace(/<\//g, "<\\/");
      page = page.replace("/*__FLDATA__*/null", () => fl).replace("/*__FLDATA__*/", () => fl);
    }
