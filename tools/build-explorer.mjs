...
const notes = [
  ...
  { id: "n_rn_depth", text: `사업개요(prjtSmry) 글자 수 중앙값 ${vals.rn_sumlen}자(전수). 추진실적은 상세 ${vals.rn_detail_n}건 기준 사업당 중앙값 ${vals.rn_prtn_med}건, 추진내용 글자 수 중앙값 ${vals.rn_prtn_len}자, 결재선 기재율 ${vals.rn_aprv_rate}%. 예산·집행 관련 필드는 목록·상세 어디에도 없음.`, sample: `${vals.rn_detail_n}건`, date: SNAPSHOT },
  { id: "n_rn_update", text: `등록일(frstRgstPot)과 최종수정일(lastUpdtPot)이 다른 사업 비율: 2024년 ${vals.rn_upd2024}%, 2025년 ${vals.rn_upd2025}%. 2025년 수정 간격 중앙값 5일 → 현행화가 아니라 등록 직후 정정.`, sample: "전수", date: SNAPSHOT },
...
const scoreboard = {
  rn: { name: "정책실명제", law: "대통령령 §63~63의5", what: "중점관리 대상사업을 골라 담당자·선정사유·추진실적을 적는 사업 단위 명부", rows: [
    ...
    ["단위당 깊이", `개요 중앙값 ${vals.rn_sumlen}자 · 추진실적 ${vals.rn_prtn_med}건×${vals.rn_prtn_len}자 · 담당자 1명+결재선. 예산 필드 없음`, "n_rn_depth"],
    ...
    ["이용", `사업당 조회 중앙값 ${vals.rn_inqmed2025}회(2025) · 2026년 ${vals.rn_inq0_2026}% 조회 0`, "n_rn_inq"],
  ...
  pv: { name: "사전정보공표", law: "정보공개법 §7 · 시행령 §4", what: "기관이 \"무엇을 어디에 언제 공개한다\"를 미리 정해 알리는 안내 목록", rows: [
    ...
    ["단위당 깊이", `세부항목 중앙값 ${vals.pv_detail_len}자 + 기관 홈페이지 링크. 내용은 포털 밖`, "n_pv_depth"],
